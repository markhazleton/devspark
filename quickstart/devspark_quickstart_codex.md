...
1. If the workspace path contains a drive letter (e.g., `C:\`) or the environment is Windows → **Windows** (active runtime: PowerShell)
2. Otherwise → **macOS/Linux** (active runtime: Bash)
...
If `/devspark.specify` does not appear in Codex, give session-scoped prompt-discovery guidance.
