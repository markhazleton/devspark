...
| `.cursor/commands/specify.*.md` files | **Old Spec Kit shims.** Rename to `devspark.*` prefix. |
...
## Step 6: Create Cursor Command Shims
...
Replace `{name}` with the actual command name (e.g., `specify`, `plan`, `implement`).
