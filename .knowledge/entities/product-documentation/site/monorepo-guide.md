...
Every monorepo declares its applications in a single authoritative file: `.knowledge/entities/application-registry/registry.json`.
...
| **Specs/Plans/Tasks** | Stored under `{app.path}/.devspark.work/specs/` for app-scoped workflows |
...
|----------|--------------|
| Repository documentation root | `.knowledge/` (root-level only, not app-local) |
