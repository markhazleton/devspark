...
The `/devspark.pr-review` command performs automated, constitution-driven code reviews for GitHub Pull Requests. It evaluates code changes against your project's established principles and writes detailed feedback reports as temporary local work state.

> **Note**: This command is **independent of the Spec-Driven Development workflow**. Unlike commands like `/devspark.specify`, `/devspark.plan`, and `/devspark.tasks`, PR review does **not** require any spec, plan, or tasks to exist. It only requires a project constitution (`/.knowledge/governance/constitution.md`) and works for any PR in any repository—whether or not you're using spec-driven development.
...
5. Save the report to `/.devspark.work/pr-reviews/pr-{number}.md`
...
- Open PRs and recently updated PR branches
...
- Update the temporary local review work file
- Use GitHub and Git as the durable review record
...
Reviews are saved to: `/.devspark.work/pr-reviews/pr-{number}.md`

Example: `/.devspark.work/pr-reviews/pr-123.md`
...
cat .devspark.work/pr-reviews/pr-456.md
...
# Analyze temporary local review output
ls .devspark.work/pr-reviews/
```
...
```text
.devspark.work/pr-reviews/pr-123.md
```
...
- New review appears at top of the local work file
- Prior local review output moves to a "Local Review Iterations" section
...
### 3. Keep Reviews Out of Commits

- Do not commit `/.devspark.work/pr-reviews/`
- Use GitHub comments and Git history as the durable review record
- Delete local review work files after findings are assimilated
...
- Link to PR comments or commits, not local `.devspark.work` files
