...
template set contains 28 active command files.
...
1. **Personal** -- `.knowledge/overrides/{git-user}/commands/`
2. **Team** -- `.knowledge/overrides/commands/`
...
Yes. Stock prompts live in `.devspark/defaults/commands/`, repo overrides live in `.knowledge/overrides/commands/`, and each agent gets thin shims that resolve personal, team, then stock prompts.
...
No. It only adds `.devspark/`, `.knowledge/`, and optionally a few agent config files. It doesn't modify your build system, CI/CD, or linter config.
