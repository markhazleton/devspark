...
def _build_full_spec_repo(repo_root: Path) -> None:
    feature_dir = repo_root / ".documentation" / "specs" / "002-full-feature"
    _write(
        feature_dir / "spec.md",
        """---
...
# Feature Specification: Full Flow
""",
    )
    _write(
        feature_dir / "tasks.md",
...
- [x] T001 Foundation complete
- [x] T002 Story one complete
""",
    )
    (repo_root / ".github").mkdir(parents=True, exist_ok=True)
    _write(repo_root / "README.md", "full spec repo\n")
...
def main() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        full_spec_repo.mkdir()
        _copy_script_set(full_spec_repo)
        _build_full_spec_repo(full_spec_repo)
        _init_git_repo(full_spec_repo, "002-full-feature")

        full_preflight = _run_powershell_preflight(full_spec_repo)
