...
├── /devspark.compliance-score command
...
### 1.1 Command Specification: `/devspark.landmark`
...
```bash
/devspark.landmark {type} [options]

# Examples
/devspark.landmark spec                    # Create spec review PR
/devspark.landmark plan                    # Create plan review PR
/devspark.landmark ready                   # Validate pre-implementation
/devspark.landmark spec --draft            # Create as draft PR
/devspark.landmark plan --reviewers @arch  # Assign specific reviewers
```
...
|------|---------------|-------------|-----------------|
| `spec` | After `/devspark.specify` | `spec.md` | Constitution alignment, scope validation |
| `plan` | After `/devspark.plan` | `plan.md`, `data-model.md`, `contracts/` | Architecture review, spec coverage |
| `ready` | Before `/devspark.implement` | Nothing (validation only) | All prior landmarks, task readiness |
...
2. **Run Readiness Checks**
   ...
   - [ ] /devspark.critic run (optional, configurable)
...
3. **Output (No PR Created)**
   ...
   Ready to implement. Run /devspark.implement to begin.
   ```
```
...
---
*Generated by /devspark.landmark spec*
```
...
---
*Generated by /devspark.landmark plan*
```

### 1.2 Command Specification: `/devspark.status`
...
```bash
/devspark.status                    # Current spec status
/devspark.status --all              # All active specs
/devspark.status 004-notifications  # Specific spec
```
...
- Continue implementing: `/devspark.implement`
- Check compliance: `/devspark.site-audit --scope=constitution`
```
...
### 2.1 Command Specification: `/devspark.harvest`
...
```bash
/devspark.harvest                           # Harvest current spec
/devspark.harvest 004-notifications         # Harvest specific spec
/devspark.harvest --dry-run                 # Preview without changes
/devspark.harvest --skip-prompts            # Use defaults (no harvest, just delete)
/devspark.harvest --force                   # Skip PR merge check (dangerous)
```
...
To override (not recommended): /devspark.harvest --force
```
...
### To Execute
/devspark.harvest 004-notifications
```
...
1. Run `/devspark.discover-constitution` on existing codebase
...
[l] Later, I'll run /devspark.harvest manually
...
#### `/devspark.site-audit` Integration
...
3. Implement `/devspark.status` command
4. Implement `/devspark.landmark spec` (spec review PR)
...
1. Implement `/devspark.landmark plan`
2. Implement `/devspark.landmark ready`
...
2. Implement `/devspark.harvest` (basic flow)
...
5. Integrate with `/devspark.release`
6. **Implement `/devspark.compliance-score` command**
...
| Clear next actions | `/devspark.status` always shows next step |
