...
If you keep overrides in `.documentation/commands/`, review any upgraded stock prompt that changed its contract. In v1.5.0 that especially includes route-aware intake and frontmatter-driven commands such as `/devspark.specify`, `/devspark.plan`, `/devspark.tasks`, `/devspark.implement`, and `/devspark.create-pr`.
