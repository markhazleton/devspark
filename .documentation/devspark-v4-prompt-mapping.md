...
| **tasks** | **New responsibility**: each generated task carries empty `code_ref` / `knowledge_ref` placeholders from the start, not added later. `tasks` sets up the linkage contract that `implement` fills and `analyze`/verify-before-delete later checks. |
...
| **quickfix** | Bypasses the full spec, but should still populate a minimal `context_resolved` and task-level `code_ref`/`knowledge_ref` for whatever it touches — "lightweight" should mean less ceremony, not exemption from the linkage that makes verify-before-delete possible. Worth deciding explicitly whether quickfix skips analyze/critic entirely or runs a lighter version of the same two checks. |

## Implementation — applies the delta

| Command | v4 guidance |
|---|---|
| **implement** | This prompt now carries the heaviest v4 discipline, matching what the philosophy calls "the cost deliberately moved to implement": (1) consumes `context_resolved` as already-resolved context — never traverses more than one hop itself; escalation past one hop is logged and attributable, not silent; (2) updates code and knowledge together in one pass, preferring test evidence over code-only evidence, recording `test_attempted`/`fallback_reason` when it falls back; (3) populates `code_ref`/`knowledge_ref` on every task as it completes it; (4) never writes a spec, task, or plan identifier into a code comment (rule 1/5); (5) only deletes the spec package after every task's linkage is verified populated — this is where verify-before-delete actually executes, not just gets checked. |
...
| **address-pr-review** | Unchanged in mechanism (commit isolation between fixes and review-file updates), but the fixes it produces are still subject to rule 1 — a fix that patches code shouldn't introduce a spec/task reference to "explain" what review comment it's addressing. |
...
| **verify** | **This is the evidence mechanism's engine, and it currently has no shims.** Its stated purpose — "produce empirical proof-of-change evidence, not document review, for declared verification modes" — is almost exactly the `verified_by: execution` discipline the philosophy specifies. This should be the prompt that actually runs cited tests and produces the pass/fail result that gets written into a knowledge object's or decision's evidence entry. Given how central evidence-by-execution is to the whole model, the missing agent/prompt shims here are the highest-priority gap in the inventory — without verify, "prefer test evidence" has no tool actually generating it. |
| **audit / site-audit** | *(Note: only `site-audit` appears in the inventory as a standing command; if a separate repo-wide `audit` is planned per the philosophy's "audit" references, it may be this command under a different name — worth confirming there's one command, not two overlapping ones.)* Repo-wide, not diff-scoped. Should run: the accuracy pass (resolve every `execution`-type evidence entry and re-run its test — existence and accuracy are separate reports, per the philosophy), and the contradiction scan, scoped to graph-adjacent objects (same entity, entities sharing a `constrains` decision, objects citing the same evidence) rather than brute-force comparison. Contradiction findings are warnings for human review, always — never a gate. |
| **harvest** | **Role shrinks under v4.** Its old purpose — "verify durable knowledge still matches the code" as a periodic sweep — is now pr-review's and audit's job, running continuously/at-merge instead of whenever someone remembers to run harvest. What's left for harvest: sweeping `.devspark.work` for orphaned or abandoned in-flight state (a spec package that stalled without completing verify-before-delete), not primary knowledge verification. Worth an explicit purpose-statement rewrite so it's not read as doing the same job as audit. |
| **archive** | Redirects to harvest — no change, but its own continued existence is worth a second look: if "archive" as a concept contradicts current-truth-only, an alias that still uses that name may be worth renaming even as a deprecated redirect, so agents encountering it don't infer archiving is a live pattern. |
...
| **release** | Should confirm, before sealing, that every spec included in the release completed verify-before-delete (task linkage populated) rather than just checking the repository delta and knowledge are current — release is a natural second checkpoint for the same guarantee pr-review already checked once. |
...
| **upgrade** | Framework-version concern, orthogonal to the knowledge model. No change. |
...
## Shim gaps, prioritized by v4 relevance

- **`devspark.verify`** — no agent or prompt shim. Highest priority: this
  command's stated purpose is the evidence-execution engine the whole
  test-evidence preference depends on. Without it, `verified_by: execution`
  has no tool that actually produces the result.
- **`devspark.address-pr-review`** — missing agent shim only (prompt shim
  exists). Lower priority than verify, but worth closing since pr-review is
  now the primary assimilation trigger and its follow-up command should be
  fully wired.
