# Implementation Plan: `/devspark.pr-review` Command
...
1. User explicitly provides PR number: `/devspark.pr-review #123` or `/devspark.pr-review 123`
...
*Review generated by devspark.pr-review | Constitution-driven code review*
...
- Show how to create one with `/devspark.constitution`
...
```markdown
| `/devspark.pr-review` | Review PRs against constitution | After PR creation, before merge |
```
...
The `/devspark.pr-review` command performs constitution-based code reviews on GitHub Pull Requests:
...
The `/devspark.pr-review` command performs automated, constitution-driven code reviews for GitHub Pull Requests. It evaluates code changes against your project's established principles and generates detailed feedback reports.
...
5. Re-run `/devspark.pr-review` to verify fixes

## Common Scenarios

### First-Time Setup

If `/devspark.pr-review` fails because constitution doesn't exist:

```bash
/devspark.constitution Create project principles for code quality, testing, and security
```
...
Create one: `/devspark.constitution`
...
For more information, see the [Spec-Driven Development Guide](https://github.com/MarkHazleton/devspark/blob/main/spec-driven.md).
...
The `/devspark.pr-review` command follows standard speckit patterns but with unique considerations:
...
The `/devspark.pr-review` command is successful when:
