...
SDD is a methodology, not a tool — and DevSpark embodies this principle through two architectural decisions:
...
Teams share constitutions, specs, and workflow governance — but individuals can customize how they interact with those workflows. The `/devspark.personalize` command creates per-user prompt overrides in `.documentation/{git-user}/commands/` that take priority over shared defaults.
...
### The `/devspark.pr-review` Command
...
### The `/devspark.site-audit` Command
...
/devspark.specify Real-time chat system with message history and user presence
...
/devspark.plan WebSocket for real-time messaging, PostgreSQL for history, Redis for presence
