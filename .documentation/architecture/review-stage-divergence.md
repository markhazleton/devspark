...
| `harvest`            | Sufficient | Shim points to canonical file; archival logic is owned by `scripts/*/harvest.*`, not the prompt. |
| `commit-audit`       | Sufficient | Shim points to canonical file; commit-mining logic is owned by `scripts/*/commit-audit-context.*`. |

No command requires bespoke prose duplicated under `templates/prompts/atomic/`.
The runner resolves the shim id, then forwards execution to the canonical
command body via the existing 3-tier override chain.
