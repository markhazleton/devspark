...
This matrix tracks the v1.6.0 release-hardening scenarios that need explicit validation before release.
...
| Upgrade from v1.5.x to v1.6.0 | Prompt/template contract alignment and quickstart repair flows preserve repo-owned artifacts while repairing incomplete framework installs | `test_upgrade_reporting.py` and `test_upgrade_migration_safety.py` validate version reporting, override warnings, and safe migration behavior; quickstart repair and prompt-alignment evidence is recorded in `.documentation/release-readiness-v1.6.0.md` | Complete |
...
| Release readiness | v1.6.0 has repository-local release evidence, release packaging notes, and GitHub release prerequisites validated | `.documentation/release-readiness-v1.6.0.md` records the validation set, release judgment, and release-publishing prerequisites | Complete |
| Current-state documentation audit | Public docs, quickstarts, templates, and examples reflect current v1.6.0 behavior | `test_documentation_audit.py` validates 69 in-scope files and `.documentation/docs-audit-2026-04-12.md` records the targeted release-surface audit | Complete |
...
- `.documentation/release-readiness-v1.6.0.md` records Step 9 release evidence.
- `.documentation/docs-audit-2026-04-12.md` records the Step 10 targeted release-surface audit.

## Remaining Validation Work

No repository-local validation gaps remain for v1.6.0.

The only excluded check is a live hosted pull-request mutation, which remains intentionally omitted from automated local validation to avoid remote side effects. That judgment is recorded in `.documentation/release-readiness-v1.6.0.md`.
